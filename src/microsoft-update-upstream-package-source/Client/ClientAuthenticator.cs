...
internal sealed class ClientAuthenticator(Endpoint endpoint, string accountName, Guid accountGuid)
{
    ...
    private async Task<UpdateServices.WebServices.DssAuthentication.AuthorizationCookie> GetAuthorizationCookieAsync(AuthPlugInInfo authInfo)
    {
        var httpBinding = new System.ServiceModel.BasicHttpBinding();
    ...
    private async Task<Cookie> GetServerAccessCookieAsync(UpdateServices.WebServices.DssAuthentication.AuthorizationCookie authCookie)
    {
        var httpBinding = new System.ServiceModel.BasicHttpBinding();
        var upstreamEndpoint = new System.ServiceModel.EndpointAddress(endpoint.ServerSyncURI);
        if (upstreamEndpoint.Uri.Scheme.Equals("https", StringComparison.OrdinalIgnoreCase))
        ...
        cookieRequest.GetCookie.authCookies = [new UpdateServices.WebServices.ServerSync.AuthorizationCookie()];
